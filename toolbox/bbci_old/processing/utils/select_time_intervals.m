...
lti= enlarge_interval(epo_r.x - opt.offset, ti, -1, opt, nfo);
uti= enlarge_interval(epo_r.x - opt.offset, ti, 1, opt, nfo);
...
if opt.visu_scalps,
  nIvals= numel(nfo);
  for ii= 1:nIvals,
    H.ax_scalp(ii)= subplotxl(2, nIvals, nIvals + ii, [0.02 0 0.02]);
  end
