function [epo, iArte, dmaxmin]= proc_rejectArtifactsMaxMin(epo, threshold, varargin)
...
if opt.verbose,
  fprintf('%d/%d artifact trials removed (max-min>%d uV), %d remained\n', ...
          length(iArte),size(epo.x,3),threshold, size(epo.x,3)-length(iArte));
end
