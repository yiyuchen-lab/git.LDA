...
[opt_orig, isdefault]= ...
    set_defaults(opt, ...
                 ...
                 'ShadeDifference', 0, ...
                 'ShadeDifferenceColor', [1 .8 .8; .8 .8 1], ...
                 'grid_over_patches', 1, ...
                 'plotStd', 0, ...
                 'stdLineSpec', '--', ...
                 'oversizePlot',1);
...
if isfield(epo, 'yUnit') && strcmp(epo.yUnit,'auc'),
...
elseif nChans>1,
  ...
  if opt.legend,
    ...
    H{1}.leg= legend(H{1}.plot, epo.className);
...
if opt.ShadeDifference,
  ...
  if size(opt.ShadeDifferenceColor,1)==1,
    H.shade= patch([epo.t(:); flipud(epo.t(:))], [xx(:,1); flipud(xx(:,2))], ...
                   opt.ShadeDifferenceColor, 'EdgeColor','none');
  else
    xx_pos= xx;
    xx_pos(:,1)= max(xx_pos(:,1), xx_pos(:,2));
    xx_neg= xx;
    xx_neg(:,2)= max(xx_neg(:,1), xx_neg(:,2));
    H.shade_pos= patch([epo.t(:); flipud(epo.t(:))], ...
                       [xx_pos(:,1); flipud(xx_pos(:,2))], ...
                       opt.ShadeDifferenceColor(1,:), 'EdgeColor','none');
    H.shade_neg= patch([epo.t(:); flipud(epo.t(:))], ...
                       [xx_neg(:,1); flipud(xx_neg(:,2))], ...
                       opt.ShadeDifferenceColor(cc,:), 'EdgeColor','none');
  end
end
...
H.hidden_objects= [];
if opt.xZeroLine,
  H.xZero= line([-1e10 1e10], [1 1]*opt.ZeroLineOffset, ...
...
if isfield(epo, 'refIval'),
  yPatch= [-1 1] * opt.refVSize * diff(yLim) + opt.ZeroLineOffset;
