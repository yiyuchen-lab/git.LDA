...
if isfield(fv,'yUnit') && strcmp(fv.yUnit,'auc'),
  if isdefault.cLim,
    opt.cLim= 'sym.5';
  end
elseif min(fv.x(:))<0,
...
if isdefault.colormap && ismember(opt.cLim,{'sym','sym.5'}),
  opt.colormap= cmap_posneg(31);
end
...
elseif strcmpi(opt.cLim, 'sym'),
  ...
  mm= max(abs(mi), ma);
  opt.cLim= [-mm mm];
elseif strcmpi(opt.cLim, 'sym.5'),
  ci= chanind(fv, clab);
  mi= min(min(fv.x(:,ci)));
  ma= max(max(fv.x(:,ci)));
  mm= max(abs(0.5-mi), abs(ma-0.5));
  opt.cLim= 0.5 + [-mm mm];
end
