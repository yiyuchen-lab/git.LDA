...
if dataInMAT(file)
    [dum file] = dataInMAT(file);
    [cnt, mrk_orig]= eegfile_loadMatlab(file);
    if  ~isempty(opt.lp_filt)   || ~isempty(opt.hp_filt)   
        warning('Filter parameters are disregarded, since MAT data was read!!!')
    end
else
...
mrk.indexedByEpochs = {'time', 'pos', 'toe'};
[epo valid]= cntToEpo(cnt, mrk, opt.disp_ival);
if  ~isempty(opt.ref_ival)
    epo= proc_baseline(epo, opt.ref_ival);
end
if length(valid) < size(mrk.y,2)
...
if opt.crit_minmax>0,
    ...
    mrk = mrk_selectEvents(mrk, iArte,'invert',1);
end
...
function [bool correctfilename] = dataInMAT(file)
dum = dir([EEG_MAT_DIR file]);
if length(dum) == 0
    bool = 0;
    correctfilename = file;
    disp('Data was not found in MAT-folder, reading RAW-data now!\n')
elseif length(dum) == 1
    bool = 1;
    correctfilename = [fileparts(file) '/' dum.name];
else 
    allFittingFiles = [dum.name]
    takenFile = dum(1).name;
    warning(sprintf('found several data sets in MAT folder: %s \n Now taking only the first match: %s', allFittingFiles, takenFile))
end
 bool = 1;
 correctfilename = [fileparts(file) '/' dum.name];    
